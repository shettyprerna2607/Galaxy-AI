...
async function chat(messages) {
  ...
  try {
    const response = await fetch(`${OLLAMA_BASE_URL}/api/chat`, {
      ...
      // Increased to 2 minutes for large code generation
    });
    if (!response.ok) {
...
async function predict(codePrefix) {
  const model = await getAvailableModel();
  try {
...
${codePrefix}`,
      ...
      // Fast timeout for inline ghost text
    });
...
class AST {
  ...
  static #parseGlob(glob2, hasMagic2, noEmpty = false) {
    ...
    for (let i = 0; i < glob2.length; i++) {
      ...
      if (c === "*") {
        re += noEmpty && glob2 === "*" ? starNoEmpty : star$1;
        hasMagic2 = true;
        continue;
      }
      if (c === "?") {
...
const fsFromOption = (fsOption) => !fsOption || fsOption === defaultFS || fsOption === fs ? defaultFS : {
  ...defaultFS,
  ...fsOption,
  promises: {
    ...defaultFS.promises,
...
let db = null;
let table = null;
const DB_PATH = path$1.join(process.env.APPDATA || (process.platform === "darwin" ? process.env.HOME + "/Library/Preferences" : process.env.HOME + "/.local/share"), "galaxy-ai/vectors");
async function initDB() {
  try {
    if (!fs__default.existsSync(path$1.dirname(DB_PATH))) {
      fs__default.mkdirSync(path$1.dirname(DB_PATH), { recursive: true });
    }
...
async function indexProject(projectPath, onProgress) {
  try {
    if (!db) await initDB();
    if (!db) return;
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      if (content.length > 5e3) continue;
      const chunks = [content.slice(0, 500)];
...
async function searchCode(query, limit = 3) {
  try {
    if (!table) return [];
...
function createWindow() {
  win = new BrowserWindow({
    ...
    webPreferences: {
      preload: fs__default.existsSync(path$1.join(__dirname$1, "preload.mjs")) ? path$1.join(__dirname$1, "preload.mjs") : path$1.join(__dirname$1, "preload.js"),
  ...
  if (VITE_DEV_SERVER_URL) {
    ...
    try {
      ...
      if (fs__default.existsSync(javaRoot)) {
        const entries = fs__default.readdirSync(javaRoot);
        const jdk = entries.sort().reverse().find((e) => e.startsWith("jdk"));
        if (jdk) {
          const binPath = path$1.join(javaRoot, jdk, "bin");
          if (fs__default.existsSync(binPath)) initialJavaPath = binPath;
        }
      }
    } catch (e) {
    }
    ...
    if (initialJavaPath && os.platform() === "win32") {
      shellProcess.stdin.write(`set PATH=${initialJavaPath};%PATH%\r
`);
      shellProcess.stdin.write(`cls\r
`);
    }
...
ipcMain.on("terminal-cd", (_, projectPath) => {
  if (shellProcess) {
    try {
      shellProcess.kill();
    } catch (e) {
  ...
  setTimeout(() => {
    ...
    if (autoJavaPath && os.platform() === "win32") {
      ...
      shellProcess.stdin.write(`${setPath}\r
`);
      shellProcess.stdin.write(`cls\r
`);
    }
    ...
    if (autoJavaPath) {
      setTimeout(() => {
        if (win && !win.isDestroyed()) {
          win.webContents.send("terminal-data", `\r
\x1B[32mEnvironment configured: ${autoJavaPath}\x1B[0m\r
`);
    ...
    if (win && !win.isDestroyed()) {
      win.webContents.send("terminal-data", `\r
\x1B[36mProject directory synchronized: ${projectPath}\x1B[0m\r
\r
`);
    }
  }, 100);
});
ipcMain.handle("get-file-tree", async (_, projectPath) => {
  ...
  try {
    const rootName = path$1.basename(projectPath);
...
ipcMain.handle("create-file", async (_, filePath) => {
  try {
    if (!fs__default.existsSync(filePath)) {
      fs__default.writeFileSync(filePath, "", "utf-8");
      return { success: true };
    }
...
ipcMain.handle("index-project", async (event, projectPath) => {
  try {
    await indexProject(projectPath, (msg) => event.sender.send("indexing-progress", msg));
    return { success: true };
  } catch (error) {
    return { success: false, error: error.message };
  }
});
ipcMain.handle("search-code", async (_, query) => {
  try {
...
ipcMain.handle("inline-predict", async (_, codePrefix) => {
  try {
    return await predict(codePrefix);
  } catch {
    return "";
  }
});
ipcMain.handle("chat", async (_, messages, projectContext) => {
  try {
...
${projectContext}`
    ...
    const aiResponse = await chat(finalMessages);
